...
const auth = async (req, res, next)=>{
    try {
       ...
       //attach the authenticated user (whether a normal user or vendor) to the request objects
...
const vendorAuth = (req, res, next) =>{
    ...
    if(!req.user.role || req.user.role!=="vendor"){
        //if the user is not vendor, return 403(Forbidden) response with an error message
...
module.exports = { auth, vendorAuth }; // Correct export